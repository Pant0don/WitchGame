...
class ActorEvents_16 extends ActorScript
{
	...
	override public function init()
	{
		...
		actor.setYVelocity(randomFloatBetween(-8, -1));
		actor.setXVelocity(randomInt(Math.floor(-4), Math.floor(4)));
		actor.growTo(0/100, 0/100, randomInt(Math.floor(.8), Math.floor(2)), Back.easeIn);
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				{
					actor.setFilter([createTintFilter(Utils.getColorRGB(153,153,153), 100/100)]);
					actor.alpha = 50 / 100;
					actor.sendToBack();
				}
